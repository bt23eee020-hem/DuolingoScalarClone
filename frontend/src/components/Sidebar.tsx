...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed bottom-0 left-0 z-20 flex h-16 w-full border-t border-gray-200 bg-white dark:bg-[#121214] dark:border-gray-800 px-2 py-1 md:bottom-auto md:left-0 md:top-0 md:h-screen md:w-64 md:flex-col md:border-r md:border-t-0 md:px-4 md:py-6">
      ...
      {/* Nav List */}
      <nav className="flex w-full items-center justify-around gap-1 md:mt-8 md:flex-col md:justify-start md:gap-2 h-full">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-4 rounded-xl px-4 py-3.5 text-[15px] font-black tracking-wider uppercase transition-all duration-250 w-full justify-center md:justify-start border-2 ${
                isActive 
        ...
        <button
          onClick={toggleTheme}
          className="flex items-center gap-4 rounded-xl px-4 py-3 text-sm font-bold tracking-wide uppercase text-gray-500 dark:text-gray-400 hover:bg-gray-50 hover:border-gray-200 dark:hover:bg-gray-800/40 border-2 border-transparent transition-all duration-200 w-full justify-center md:justify-start md:mt-auto cursor-pointer"
        >
          {isDarkMode ? (
            <>
        ...
        <button
          onClick={logout}
          className="flex items-center gap-4 rounded-xl px-4 py-3 text-sm font-bold tracking-wide uppercase text-gray-500 dark:text-gray-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/20 dark:hover:text-red-400 transition-all duration-200 w-full justify-center md:justify-start cursor-pointer border-2 border-transparent"
        >
